...
do
  ...
  echo "Quantifying reads for $BASE_NAME" >> batch_salmon.log

  if [ -z "$READ2_SUFFIX" ]
  then
    READ_ARGUMENT="-r $IN_NAME1"
    echo "Using default '--fldMean 250' and '--fldStd 25' values for single-end fragment size. Correct these as needed!!!" >> batch_salmon.log
  else
  ...
  then
    ...
    echo "Will use $LIB_TYPE as the library type for all remaining files" >> batch_salmon.log
  fi

  echo "Finished quantifying reads for $BASE_NAME" >> batch_salmon.log
