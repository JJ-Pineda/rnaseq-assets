...
do
  ...
  then
    ...
    bbsplit.sh -Xmx10g threads=12 build=$BUILD path="$INDEX_PATH" maxindel=$MAXINDEL ambiguous2=$AMBIG2 in="$IN_NAME" basename="${BASE_NAME}_%.fastq.gz" refstats="${BASE_NAME}_stat.txt"
  else
    ...
    bbsplit.sh -Xmx10g threads=12 build=$BUILD path="$INDEX_PATH" maxindel=$MAXINDEL ambiguous2=$AMBIG2 in1="$IN_NAME1" in2="$IN_NAME2" basename="${BASE_NAME}_%_#.fastq.gz" refstats="${BASE_NAME}_stat.txt"
  fi
